      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between my-5 ">
          {steps.map((step, index) => (
            ...
            >
              <span
                className={`pi pi-check-square ${"hidden max-sm:flex"}`}
                style={{ fontSize: "1.5rem" }}
              />
              <h2
                className={`text-lg font-bold max-sm:hidden bg-gray-300 text-black px-3 py-1 rounded-full ${activeStep === index || activeStep >= index ? "bg-green-500 text-foreground-300" : ""}`}
              >
                {index + 1}
              </h2>
              <span
                className={`max-sm:hidden ${activeStep === index || activeStep >= index ? "text-green-500" : ""}`}
              >
                {step.label}
              </span>
            </div>
        ...
        <Card className=" shadow rounded-lg">
          ...
          {/* <CardBody className="px-4 py-6">
            <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
              <Stepper activeStep={activeStep} orientation="horizontal" headerPosition="bottom">
                {steps.map((step, index) => (
                  <StepperPanel key={index} header={step.label} ptOptions={CardBody}>
                    <div className="px-4 py-6">{renderStepContent(index)}</div>
                  </StepperPanel>
                ))}
          </Stepper>
            </form>
          ...
          <CardFooter className="flex flex-row justify-between items-center px-4 py-3 border-t">
            {activeStep > 0 && (
              <Button
                className="mb-0"
                color="secondary"
