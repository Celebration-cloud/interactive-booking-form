        ...
        <div className="flex justify-between my-5 max-sm:hidden">
          {steps.map((step, index) => (
            <div key={index} className="text-center flex flex-col items-center ">
              <h2 className={`text-lg font-bold bg-green-400 px-3 py-1 rounded-full ${activeStep === index ? "bg-foreground-700 text-foreground-300" : ""}`}>{index + 1}</h2>
              <span cl>{step.label}</span>
            </div>
          ))}
          {/* <Steps
          readOnly
